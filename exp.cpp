...
inline void validate_input(int argc, char *argv[], std::int64_t& runs,
    std::int64_t& lower, std::int64_t& upper, std::int64_t& step, std::int64_t& threads)
{
    if (argc < 7) {
        std::cerr << "Usage: <filename> <RUNS> <LOWER> <UPPER> <STEP> <sec/par> <threads>" << std::endl;
...
int main(int argc, char *argv[])
{
    ...
    std::int64_t runs, lower, upper, step, threads;
    validate_input(argc, argv, runs, lower, upper, step, threads);
    std::string paralelo = argv[6];
    paralelo == "par" ? threads = std::stoll(argv[7]) : threads = 1;
