...
inline void validate_input(int argc, char *argv[], std::int64_t& runs,
    std::int64_t& lower, std::int64_t& upper, std::int64_t& step, std::int64_t& k, std::int64_t& threads)
{
    if (argc < 8) {
        std::cerr << "Usage: <filename> <RUNS> <LOWER> <UPPER> <STEP> <sec/par> <k> <threads>" << std::endl;
        ...
        std::cerr << "<sec/par>: choose between 'sec' for sequential and 'par' for parallel implementation" << std::endl;
        std::cerr << "<k>: number of ways for k-way merge sort." << std::endl;
        std::cerr << "<threads>: number of threads for parallel implementation." << std::endl;
...
int main(int argc, char *argv[])
{
    ...
    std::int64_t runs, lower, upper, step, k, threads;
    validate_input(argc, argv, runs, lower, upper, step, k, threads);
    std::string paralelo = argv[6];
    paralelo == "par" ? threads = std::stoll(argv[8]) : threads = 1;
    ...
    time_data << "n,t_mean,t_stdev,t_Q0,t_Q1,t_Q2,t_Q3,t_Q4,k";
    if (paralelo == "par") {
        time_data << ",threads";
    }
    time_data << std::endl;
    ...
    for (n = lower; n <= upper; n += step) {
        ...
        for (i = 0; i < runs; i++) {
            ...
            begin_time = std::chrono::high_resolution_clock::now();
                ...
                if (paralelo == "sec") {
                    sequentialCompleteMergeSort(A, 0, N - 1, k);
                } else if (paralelo == "par") {
                    omp_set_num_threads(threads);
                    parallelCompleteMergeSort(A, k);
                } else {
                    std::cerr << "Invalid algorithm choice. Use 'sec' for sequential or 'par' for parallel." << std::endl;
                    std::exit(EXIT_FAILURE);
                }
        ...
        time_data << q[0] << "," << q[1] << "," << q[2] << "," << q[3] << "," << q[4] << "," << k;
        if(paralelo == "par") {
            time_data << "," << threads;
        }
        time_data << std::endl;
